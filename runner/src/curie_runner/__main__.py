...
def build_runner(
    ...
    *,
    ...
    # ADR-0115: same absent-means-unmounted pattern as the state client above.
    ...
    def factory() -> ModelSession:
        if fake_model:
            ...
            return FakeModelSession(
                ...
                # ADR-0115: a scripted [fake:delegate:<agent>] marker makes a
                # REAL delegate call through this client, offline.
                delegate_client=delegate_client,
                # ADR-0115: on the TARGET side of a delegate call, answer a
                # delegated arithmetic question deterministically instead of
                # returning the canned "all done", which reads as a non-answer
                # to whatever was actually asked.
