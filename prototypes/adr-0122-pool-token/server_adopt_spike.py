...
# keeps working.
_GATED_PATHS = frozenset({"/v1/event", "/v1/steer", "/v1/interrupt", "/v1/reset", "/v1/snapshot", "/v1/adopt"})
...
class _TokenHolder:
    """SPIKE: one active credential, swappable at runtime."""

    def __init__(self, token: str) -> None:
        self._b = token.encode("utf-8")
        self.rotations = 0

    def check(self, presented: str) -> bool:
        return hmac.compare_digest(presented.encode("utf-8"), self._b)

    def rotate(self, new: str) -> None:
        self._b = new.encode("utf-8")
        self.rotations += 1


async def _adopt(request: web.Request) -> web.StreamResponse:
    """SPIKE: swap the accepted credential. Gated, so only the current holder can."""
    holder = request.get("token_holder")
    if holder is None:
        return web.json_response({"error": "no token configured"}, status=409)
    try:
        body = await request.json()
    except Exception:
        return web.json_response({"error": "bad json"}, status=400)
    new = (body or {}).get("token")
    if not isinstance(new, str) or not new:
        return web.json_response({"error": "token required"}, status=400)
    holder.rotate(new)
    return web.json_response({"ok": True, "rotations": holder.rotations})
...
def _auth_middleware(token: str) -> Middleware:
    ...
    # SPIKE (ADR-0122 unresolved #3): the token was invariant for the process.
    # A holder makes it swappable so /v1/adopt can retire a bootstrap.
    ...
    async def middleware(request: web.Request, handler: Handler) -> web.StreamResponse:
        if request.method == "POST" and request.path in _GATED_PATHS:
            ...
            if not holder.check(presented):
                return web.json_response({"error": "invalid token"}, status=401)
            request["token_holder"] = holder
