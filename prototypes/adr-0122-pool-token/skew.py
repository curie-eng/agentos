...
print(f"    canonical value now: {G}{canon}{X}")
...
OUT["concurrent"] = {"winners": len(won), "loser_already_exists": already, "canonical": canon}
...
print(f"    pod {pod} booted with TOKEN={G}{before}{X}")
...
stale = (after == before)
print(f"    {(G+'PASS'+X) if stale else (R+'FAIL'+X)}  running pod still sees {Y}{after}{X} "
...
print(f"    {(G+'PASS'+X) if fresh else (R+'FAIL'+X)}  replacement pod {newpod} sees {G}{newval}{X}")
OUT["skew"] = {"before": before, "after_rotate_same_pod": after, "replacement": newval,
...
if stale and fresh:
    ...
    print(f"    old pods still accept {Y}{before}{X}, new pods accept {G}ROTATED-GEN-TWO{X}")
