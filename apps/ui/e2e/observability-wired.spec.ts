...
test("an agent's View traces opens the Traces list pre-filtered to that agent", async ({ page }) => {
  const agent = { id: "ag-77", name: "billing-bot", channel: { kind: "slack", address: "C0BILL" }, channels: [{ kind: "slack", address: "C0BILL" }], created_at: "2026-07-05T00:00:00Z" };
...
test("Memory tab lists an agent's learned memory with provenance and supports delete", async ({ page }) => {
  const agent = { id: "ag-mem", name: "deal-desk", channel: { kind: "slack", address: "#revenue-ops" }, channels: [{ kind: "slack", address: "#revenue-ops" }], created_at: "2026-07-05T00:00:00Z" };
