...
def _thread_key(thread: str, *, kind: str = "slack", channel: str = "C1") -> str:
    ...
    An adapter conversation id is unique only per address, so the sandbox route
    and the approval-card slot are keyed by the (kind, address, conversation)
    triple. Spelled out here rather than imported from the kernel: a test that
    reached for the production helper would agree with any format it invented,
    including the bare id these keys used to carry. The defaults mirror
    ``_qevent``'s reply handle above.
    """
