...
def _thread_key(thread: str, *, kind: str = "slack", channel: str = "C1") -> str:
    ...
    Spelled out here rather than imported from the kernel: a test that reached
    for the production helper would agree with any format it invented, including
    a bare conversation id -- which is the identity #1459 exists to end. The
...
async def _route_key(async_redis, thread: str) -> str:
    ...
    keys = [k async for k in async_redis.scan_iter(match=f"*:route:{key}")]
    assert len(keys) == 1, f"expected one route key for {key}, found {keys}"
    return keys[0]
...
def test_claim_latency_is_logged(make_harness, caplog) -> None:
    ...
    async def go() -> None:
        async with make_harness() as h:
            ...
            # "claim latency for <thread key>: <N> ms" -- non-negative integer duration.
...
def test_lock_acquire_timeout_retries_in_process(make_harness) -> None:
    ...
    async def go() -> None:
        async with make_harness(lock_acquire_timeout_s=0.2, max_attempts=3) as h:
            thread = "tLockRetry"
            lock_key = h.config.lock_key(_thread_key(thread))
