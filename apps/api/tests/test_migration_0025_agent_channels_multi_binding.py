"""Migration 0025 drops the one-binding-per-agent constraint (ADR-0107, #1459).

`0025_agent_channels_multi_binding.py` drops `agent_channels_agent_id_key`
...
# would make "-1" stop short of undoing 0025 and the test would go green
...
def test_upgrade_drops_only_the_agent_id_constraint(
    ...
    """[FAIL-FIRST] Upgrade must remove ONLY the agent_id uniqueness (ADR-0107:
...
def test_two_bindings_for_one_agent_insert_after_upgrade(
    ...
    Also proves the negative first: against the pre-0025 schema, the second
...
def test_a_duplicate_pair_still_raises_after_upgrade(
    ...
    address) after 0025 lands -- proven by execution, per AGENTS.md's
    ...
    (0023 established it, two revisions before this one). Nothing 0025 adds
    is what makes this pass; only a REGRESSION in 0025 -- a migration that
