...
def test_the_map_no_longer_carries_the_retired_one_binding_per_agent_entry() -> None:
    ...
    ADR-0107 reverses, so a reader auditing what this API refuses would conclude
    ...
    assert "agent_channels_agent_id_key" not in _UNIQUE_CONSTRAINT_MESSAGES, (
        "the one-binding-per-agent constraint is dropped by migration 0025 "
        "(ADR-0107); its 409 message can never fire again and tells an operator "
