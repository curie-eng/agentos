...
(#1459). Cardinality stopped being 1:1 in ADR-0107 (#1525), which amends
...
def test_patch_onto_taken_address_is_409(
    ...
    (ADR-0107), so the move goes there; the 409 it must answer is unchanged."""
...
def test_a_non_slack_kind_binds_and_reads_back_through_the_api(
    ...
    The read-back asserts the binding surface is a LIST. ADR-0107 amends
...
def test_a_plural_channels_payload_is_rejected(
    ...
    """CREATE binds exactly one channel, even under ADR-0107.
    ...
    ADR-0107: the message must now point at the subresource, because "send the
...
def test_patching_a_binding_moves_it_rather_than_adding_a_second(
    ...
    """T4, rewritten for ADR-0107: the two verbs stay distinct.
...
def test_an_explicit_null_channel_is_rejected_on_patch(
    ...
    """Edge case E1, still 422 but for a stronger reason under ADR-0107.
...
def test_a_singular_channel_patch_is_rejected_not_silently_ignored(
    ...
    ADR-0107, and PATCH is where saying so matters. Pydantic's default
...
def test_the_published_agent_update_has_no_channel_property_at_all() -> None:
    ...
    `null` as a value the server refused. ADR-0107 removes the field outright,
    ...
    assert "channel" not in update, (
        ...
        f"(ADR-0107). Published properties: {sorted(update)}"
    )
