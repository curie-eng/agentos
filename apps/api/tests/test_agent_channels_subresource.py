...
ADR-0107 amends ADR-0089's "one agent binds one channel" clause: an agent now
        ...
        try:
            async with engine.connect() as conn:
                result = await conn.execute(
                    sql_text(
                        "SELECT id, kind, address, generation FROM curie.agent_channels "
...
def test_adding_a_pair_this_agent_already_holds_conflicts_naming_this_agent(
    client: TestClient, auth_headers: dict[str, str], clean_db: None
) -> None:
    """[FAIL-FIRST] The savepoint's proof (plan finding 2).

    The status code is identical to the real-conflict case above, so the MESSAGE
    is the whole deliverable: an operator told "another agent is already bound"
    goes looking for an agent that does not exist, and the CLI's ensure-bound
    recheck cannot tell a satisfied desired-state from a genuine collision.

    That owner-accurate sentence can only be produced by a query that runs AFTER
    the `IntegrityError`, which is why this is also the transaction-recovery
    test: a handler that recovers with `session.rollback()` instead of a
    SAVEPOINT loses the outer transaction (and its `FOR UPDATE` locks), and a
    handler that recovers with neither answers 500 `PendingRollbackError` on the
    owner lookup. Both fail here.

    Assertion (b) is not decoration: without it a handler that swallowed the
    error and returned a 409 out of thin air would pass (a) while having left a
    partial row, or a poisoned session, behind.
    """
    ...
    # (a) 409, and the sentence names THIS agent rather than a phantom one.
    assert duplicate.status_code == 409, duplicate.text
    detail = duplicate.json()["detail"]
    assert "this agent" in detail, detail
    assert "another agent" not in detail, detail
    assert "slack:C0EXAMPLE2" in detail, detail

    # (b) The session survived the failed INSERT and the binding set is
    # unchanged -- no partial row, no poisoned transaction.
    assert _channels(client, auth_headers, agent_id) == [
        _slack("C0EXAMPLE1"),
        _slack("C0EXAMPLE2"),
    ]
...
def test_two_concurrent_adds_of_the_same_pair_yield_one_201_and_one_409(
    client: TestClient,
    rival_client: TestClient,
    auth_headers: dict[str, str],
    clean_db: None,
) -> None:
    """[FAIL-FIRST] The savepoint under real contention.
    ...
    about an unlocked world. Neither is a 409 with the owner-accurate sentence.

    The 409 must be a CLASSIFIED conflict: a 500 leaking the driver error also
    "fails the request", and the CLI's ensure-bound recheck cannot act on it.
    """
    ...
    assert "PendingRollbackError" not in loser.text, loser.text
    assert "this agent" in detail, detail
    assert "another agent" not in detail, detail
    assert "slack:C0EXAMPLE2" in detail, detail
