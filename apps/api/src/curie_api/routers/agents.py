...
_UNIQUE_CONSTRAINT_MESSAGES = {
    ...
    # No entry for `agent_channels_agent_id_key`: migration 0025 drops that
    # constraint (ADR-0107), so its message can never fire again, and it said
...
async def update_agent(agent_id: uuid.UUID, data: AgentUpdate, session: SessionDep) -> AgentOut:
    # No binding key here since ADR-0107: an agent may hold several bindings, so
...
# --- the channel-binding subresource (ADR-0107, #1525) ------------------------
#
