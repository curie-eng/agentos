...
async def create_agent(session: AsyncSession, data: AgentCreate) -> Agent:
    agent = Agent(
        ...
        # A create binds exactly ONE channel (ADR-0107 keeps the create
...
async def update_channel_binding(
    session: AsyncSession, binding: AgentChannel, channel: ChannelBindingWrite
) -> AgentChannel:
    """Move ONE binding row to a new kind/address (ADR-0096, #1459; ADR-0107).
...
async def add_channel_binding(
    ...
    """Append a binding to an agent (ADR-0107). Appends; never moves.
