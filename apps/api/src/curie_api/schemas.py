...
def _reject_retired_binding_keys(data: Any) -> Any:
    ...
    - `channels` (plural) is not a CREATE field: a create binds exactly one
      ...
      `/agents/{id}/channels` subresource (ADR-0107), so the plural key here
    ...
    if isinstance(data, dict):
        ...
        if "channels" in data:
            raise ValueError(
                ...
                "/agents/{id}/channels (ADR-0107). Creating with no binding at "
...
def _reject_retired_update_binding_key(data: Any) -> Any:
    """Refuse `channel` on an agent UPDATE (ADR-0107, #1525).
    ...
    if isinstance(data, dict) and "channel" in data:
        raise ValueError(
            ...
            "bindings (ADR-0107), so moving 'the' binding has no referent. Use "
...
class ChannelBinding(BaseModel):
    ...
    An agent holds ONE OR MORE of these (ADR-0107, #1525, amending ADR-0089's
...
class AgentUpdate(BaseModel):
    ...
    # No binding key, in either number (ADR-0107, #1525). The binding's write
...
class AgentOut(BaseModel):
    ...
    id: uuid.UUID
    name: str
    # Serialized from the plural `Agent.channels` relationship (ADR-0107).
