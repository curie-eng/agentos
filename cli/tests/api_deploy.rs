...
fn route(method: &str, path: &str) -> Response {
    match (method, path) {
        ...
        // `AgentCreate` still carries the SINGULAR `channel` on the way in
        // (ADR-0107 keeps creation one-channel); `AgentOut` answers with the
        // plural `channels` array.
...
/// `channels` key (ADR-0107). `repo` emits the `repo_full_name` key only when
...
/// The path the channel subresource lives at (ADR-0107, S3): a binding is
...
async fn redeploy_with_a_new_channel_adds_a_binding() {
    // ENSURE-BOUND (ADR-0107, D3): an agent already on C0EXAMPLE1 plus
...
async fn deploy_binds_an_unbound_agents_repo() {
    ...
    // `AgentUpdate.channel` is retired (ADR-0107): bindings move through the
...
async fn deploy_binds_the_repo_while_also_adding_the_channel() {
    // Two writes now, not one (ADR-0107): the binding add is a POST to the
