...
fn fanout_response(
    ...
    match (req.method.as_str(), req.path.as_str()) {
        ("POST", "/deploy-targets/list") => Response::json(
            200,
            &json!({"targets": [
                {"name": "dev", "agent": "acme-dev", "env": "dev", "slack_channel": "C000000D01"},
                {"name": "prod", "agent": "acme-prod", "env": "prod", "slack_channel": "C000000P01"}
            ]})
            .to_string(),
        ),
        ("POST", "/deploy-targets/resolve") => {
            ...
            Response::json(
                ...
                &json!({
                    ...
                    "env": target,
                    "slack_channel": if target == "dev" { "C000000D01" } else { "C000000P01" }
                })
                .to_string(),
            )
        }
        ("GET", "/agents") => Response::json(
            200,
            &json!([
                target_agent("acme-dev", dev_repo, "C000000D01"),
                target_agent("acme-prod", prod_repo, "C000000P01")
            ])
            .to_string(),
        ),
...
fn run_fanout(
    ...
    fs::write(
        ...
        "targets:\n  dev: { agent: acme-dev, env: dev, slack_channel: C000000D01 }\n  prod: { agent: acme-prod, env: prod, slack_channel: C000000P01 }\n",
    )
