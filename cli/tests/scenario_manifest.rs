...
/// The plan is fully resolvable offline, so `--dry-run` must print it and touch
/// neither Docker nor the network. Asserting on the PLAN payload (no `error`
/// key) is what stops an error-path regression greening this: the centralized
/// error emit would also satisfy "emits an object".
#[test]
fn dry_run_prints_the_plan_and_invokes_neither_docker_nor_the_runner() {
    let dir = tempfile::tempdir().expect("tempdir");
    let (fake_bin, docker_log) = recording_docker(dir.path());
    let manifest = manifest_from(dir.path(), "skill-two-probes.json");

    let output = run_scenario(&manifest, &fake_bin, &["--dry-run", "--json"]);

    assert_eq!(
        output.status.code(),
        Some(0),
        "a dry run is hermetic and must exit 0\n{}",
        output_text(&output)
    );
    let value: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap_or_else(|e| {
        panic!(
            "--dry-run --json must emit one JSON object: {e}\n{}",
            output_text(&output)
        )
    });
    assert!(
        value.get("error").is_none(),
        "must be the PLAN, not an error object: {value}"
    );
    assert_eq!(value["dry_run"], serde_json::json!(true), "{value}");
    let plan = value["plan"]
        .as_array()
    ...
    let joined = plan
        ...
        .join("\n");
    assert!(
        joined.contains("ac1-positive") && joined.contains("ac1-negative"),
        "the plan must name one line per probe: {joined}"
    );
    assert!(
        joined.contains("skill"),
        "the plan must name the selected tier: {joined}"
    );
    assert!(
        !docker_log.exists(),
        "a dry run must never shell out to docker: {}",
        fs::read_to_string(&docker_log).unwrap_or_default()
    );
}
