//! Integration: the `channels` verb at BOTH tiers (ADR-0107, #1525).
//!
...
//! `cluster_channels_rejects_add_and_remove_before_resolving_the_connection`,
...
/// One agent's wire JSON with its plural `channels` array (ADR-0107).
...
#[test]
fn local_channels_add_posts_the_pair_to_the_subresource() {
    let server = api(|m, p| {
...
#[test]
fn local_channels_remove_deletes_the_pair() {
    let server = api(|m, p| {
...
#[test]
fn local_channels_list_reads_without_writing() {
...
#[test]
fn local_channels_json_carries_the_agent_and_its_bindings() {
    let server = api(|m, p| {
        (m == "POST" && p == channels_path())
            .then(|| Response::json(201, &agent_json(&[BOUND, OTHER])))
    });
    ...
    // guessing it, which is the whole shape ADR-0107 introduced.
    assert_eq!(
        value["channels"],
        serde_json::json!([
...
#[test]
fn local_channels_surfaces_a_409_when_removing_the_last_binding() {
...
#[test]
fn channels_rejects_add_and_remove_together_with_exit_2_and_no_http() {
    ...
    // test would pass against a binary that has no `channels` verb at all --
...
#[test]
fn cluster_channels_add_posts_the_pair_to_the_subresource() {
...
#[test]
fn cluster_channels_rejects_add_and_remove_before_resolving_the_connection() {
    ...
    let output = Command::new(bin())
        ...
        .output()
        .expect("run curie cluster channels");
