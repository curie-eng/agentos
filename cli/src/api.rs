...
/// One of an agent's channel bindings (ADR-0107, #1525): `kind` names the
...
pub struct Agent {
    ...
    /// server-side. One or more (ADR-0107): the API refuses to remove the last
...
pub enum ChannelOutcome {
    ...
    /// deploy only ever ADDS (ADR-0107), so the agent's other channels stay
    /// routed.
...
fn add_channel_body(kind: &str, address: &str) -> serde_json::Value {
    json!({"kind": kind, "address": address})
}
...
impl ApiClient {
    ...
    pub async fn add_agent_channel(
        ...
        let resp = self
            ...
            .header("X-API-Key", &self.api_key)
            .json(&add_channel_body(kind, address))
            .send()
            .await
    ...
    async fn resolve_agent(
        ...
        match existing {
            Some(agent) => {
                ...
                // ENSURE-BOUND (ADR-0107): `--slack-channel` states a binding
                ...
                // (ADR-0107), so the repo bind stays a separate PATCH. A
                ...
                let agent = match channel_add {
                    Some(address) => self.add_agent_channel(&agent.id, "slack", address).await?,
                    None => agent,
                };
