...
| `curie cluster info <agent>` | The deployed-tier report, identical in shape and discovery pass to `curie local info <agent>`, against the release's platform API. Adds `--namespace` and `--release` so the API URL and key are discovered from the installed release when `--api-url` / `--api-key` are omitted; `--dry-run` prints the read-only plan. Same exit contract (bundle and deployment gaps are diagnoses at exit 0) and the same `--check-mcp` decline at exit 4, for the same by-construction reason: the probe needs a bundle directory on this machine and a deployed bundle has none. |
...
| 4    | unsupported | The verb was understood, but the concept it inspects does not exist at this tier by construction (`curie skill versions`, `curie skill memory`, and the flag-level `curie local info --check-mcp` / `curie cluster info --check-mcp`). No input and no retry changes that -- the same argv never succeeds here; the `fix` hint names the tier that does answer it. |
