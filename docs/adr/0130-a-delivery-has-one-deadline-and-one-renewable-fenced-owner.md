# 130. A delivery has one deadline and one renewable fenced owner
...
[the long-turn lease spike](../../.projects/spikes/sre-long-turn-lease.md)
observed all three boundaries. A runner that continued to emit progress was
cancelled at 600 seconds. A pending entry was not reclaimable immediately before
900 seconds and was reclaimable immediately after it. At that boundary a second
replica entered the handler while the original handler remained live. Raising
the runner timeout alone would therefore make the overlap larger, not make a
thirty-minute delivery safe.
